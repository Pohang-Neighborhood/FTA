...
function applyAutomaticTargetSpacing(states, intents, config) {
  ...
  for (const team of ["home", "away"]) {
    ...
    for (const [playerId, intent] of teamIntents) {
      ...
      let targetM = intent.targetM;
      for (const [acceptedId, acceptedIntent] of spaced) {
        if (statesById.get(acceptedId)?.player.team !== team) {
          continue;
        }
        const separationM = distance(targetM, acceptedIntent.targetM);
        if (separationM >= config.automaticMinimumSpacingM) {
          continue;
        }
        const fallbackDirection =
          playerId.localeCompare(acceptedId) < 0 ? -1 : 1;
        const direction =
          separationM <= EPSILON
            ? point(fallbackDirection, 0)
            : normalize(subtract(targetM, acceptedIntent.targetM));
        targetM = clampMeters(
          add(
            targetM,
            scale(
              direction,
              config.automaticMinimumSpacingM - separationM,
            ),
          ),
          config,
        );
...
function automaticIntents(
  ...
  const attackingTactics = deriveTeamTacticalState({
    ...
    possessionElapsedMs: phaseElapsedMs,
  });
  ...
  for (
    ...
    frames.push(
      createFrame(
        ...
        config,
        nextElapsedMs - possessionPhaseStartedAtMs,
      ),
    );
  }
...
function compileSequenceSimulation(scenario, sequences, config) {
  ...
  if (activeSequenceIndex < sequences.length) {
    ...
    timeline.endSnapshot = createFrame(
      ...
      [],
      config,
      scenario.durationMs - possessionPhaseStartedAtMs,
    );
