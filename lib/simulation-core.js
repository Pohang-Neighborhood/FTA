...
function offsideTrapIsActive(states, team) {
  const automaticBackLine = states.filter(
    (state) =>
  ...
  return (
    automaticBackLine.length >= 3 &&
    automaticBackLine.every((state) => state.behavior === "offside-line")
  );
}
...
function createFrame(
  ...
  const frameTactics = (team) => {
    ...
    const offsideTrapActive =
      requested.offsideTrapRequested && offsideTrapIsActive(states, team);
    if (offsideTrapActive || !requested.offsideTrapRequested) {
      return { ...requested, offsideTrapActive };
    }
