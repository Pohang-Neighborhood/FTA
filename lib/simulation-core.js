...
function normalizeScenarioSequences(scenario) {
  ...
  for (const [sequenceIndex, sequence] of sequences.entries()) {
    ...
    assertFiniteNumber(sequence.startAtMs, `${label}.startAtMs`);
    if (
      sequence.startAtMs < 0 ||
      sequence.startAtMs % SIMULATION_TICK_MS !== 0
    ) {
      throw new RangeError(
        `${label}.startAtMs must be a non-negative simulation tick.`,
      );
    }
    if (sequence.startAtMs >= scenario.durationMs) {
      throw new RangeError(`${label}.startAtMs must be inside the simulation.`);
    }
    ...
    for (const [
      ...
      if (sequence.startAtMs + instruction.atMs >= scenario.durationMs) {
        throw new RangeError(
  ...
  if (sequences[0].startAtMs !== 0) {
    throw new RangeError("The first sequence must start at 0ms.");
  }
  for (let index = 1; index < sequences.length; index += 1) {
    if (sequences[index].startAtMs <= sequences[index - 1].startAtMs) {
      throw new RangeError("Sequence startAtMs values must be strictly increasing.");
    }
  }
  for (const [index, sequence] of sequences.entries()) {
    if (sequence.order !== index + 1) {
      throw new RangeError(
        `sequences[${index}].order must be ${index + 1} after chronological sorting.`,
      );
    }
  }
  ...
  for (let index = 0; index < sequences.length - 1; index += 1) {
    ...
    for (const instruction of sequence.instructions) {
      if (sequence.startAtMs + instruction.atMs >= nextSequence.startAtMs) {
        throw new RangeError(
          `Sequence ${sequence.id} instructions must dispatch before ${nextSequence.id} starts.`,
        );
      }
    }
  }
  ...
  return sequences
    .flatMap((sequence) =>
      sequence.instructions.map((instruction) => ({
        ...instruction,
        atMs: sequence.startAtMs + instruction.atMs,
      })),
    )
...
function normalizeScenarioProgram(scenario) {
  ...
  if (hasSequences) {
    ...
    return normalizeScenarioSequences(scenario);
  }

  return normalizeScenarioInstructions(scenario);
}
...
export function compileSimulation(scenario, configOverrides) {
  ...
  const instructions = normalizeScenarioProgram(scenario);
  validateScenario(scenario, instructions, config);
  ...
  const states = scenario.players
    .map((player) =>
      createPlayerState(
        ...
        motionInstructionsByPlayerId.get(player.id) ?? [],
        config,
      ),
    )
    .sort(compareById);
  ...
  let ball = scenario.initialBallPosition
    ? {
        kind: "loose",
  ...
  for (
    ...
    frames.push(
      createFrame(
        ...
        states,
        ball,
        [...ballStep.events, ...instructionStep.events],
        config,
      ),
    );
  }
