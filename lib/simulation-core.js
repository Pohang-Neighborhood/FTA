...
function defendingAutomaticIntents(
  ...
  const outfield = defendingStates
    ...
    .sort((left, right) => {
      const pressureDistance = (state) =>
        ...
        (teamTactics.offsideTrapActive && isBackLinePlayer(state.player)
          ? 20
          : 0);
      return (
  ...
  for (const state of defendingStates) {
    ...
    if (isBackLinePlayer(state.player)) {
      intents.set(state.player.id, {
        behavior: teamTactics.offsideTrapActive
          ? "offside-line"
          : "defensive-line",
        intensity: 0.66,
...
function createFrame(
  ...
  const tactics = {
    home: deriveTeamTacticalState({
      team: "home",
      possessionTeam,
      ballPosition,
      possessionTransitionElapsedMs,
      offsideTrapEligible:
        defendingTeam === "home" && offsideTrapEligible,
    }),
    away: deriveTeamTacticalState({
      team: "away",
      possessionTeam,
      ballPosition,
      possessionTransitionElapsedMs,
      offsideTrapEligible:
        defendingTeam === "away" && offsideTrapEligible,
    }),
  };
