...
import {
  ...
  tacticalLevelValue,
  validatePlayerTacticalRole,
} from "./player-tactical-role.js";
import { deriveTeamTacticalState } from "./team-tactical-phase.js";
...
export const DEFAULT_SIMULATION_CONFIG = Object.freeze({
  ...
  automaticDecisionIntervalMs: 250,
  automaticTargetDeadbandM: 1.25,
  automaticRoleMinimumHoldMs: 750,
  automaticMinimumSpacingM: 1.8,
});
...
function tacticalRole(state) {
  return state.player.tacticalRole;
}

function tacticalLevelMultiplier(level) {
  return [0.72, 1, 1.28][tacticalLevelValue(level)];
}

function tacticalRangeM(level, axis) {
  const ranges =
    axis === "lateral"
      ? [5, 11, 19]
      : [7, 16, 27];
  return ranges[tacticalLevelValue(level)];
}

function constrainRoleTarget(state, targetM, config, overrides = {}) {
  const role = tacticalRole(state);
  const lateralRangeM = tacticalRangeM(
    overrides.lateralRange ?? role.lateralRange,
    "lateral",
  );
  const verticalRangeM = tacticalRangeM(
    overrides.verticalRange ?? role.verticalRange,
    "vertical",
  );
  return clampMeters(
    point(
      clamp(
        targetM.x,
        state.anchorM.x - lateralRangeM,
        state.anchorM.x + lateralRangeM,
      ),
      clamp(
        targetM.y,
        state.anchorM.y - verticalRangeM,
        state.anchorM.y + verticalRangeM,
      ),
    ),
    config,
...
function preferredLaneX(state, config) {
  const role = tacticalRole(state);
  const centerX = config.pitchWidthM / 2;
  const side = state.anchorM.x < centerX ? -1 : 1;
  if (role.preferredZone === "wide") {
    return centerX + side * (config.pitchWidthM * 0.43);
  }
  if (role.preferredZone === "half-space") {
    return centerX + side * (config.pitchWidthM * 0.2);
  }
  if (role.preferredZone === "central") {
    return centerX;
  }
  return state.anchorM.x;
}

function applyPreferredLane(state, targetM, config, weight = 0.46) {
  const role = tacticalRole(state);
  const nextX =
    role.preferredZone === "balanced"
      ? targetM.x
      : targetM.x + (preferredLaneX(state, config) - targetM.x) * weight;
  return constrainRoleTarget(state, point(nextX, targetM.y), config);
}

function attackingProgress(state, team, config) {
  return team === "home"
    ? 1 - state.positionM.y / config.pitchLengthM
    : state.positionM.y / config.pitchLengthM;
}

function automaticBallAction(state, team, config) {
  const role = tacticalRole(state);
  const progress = attackingProgress(state, team, config);
  const centerDistance = Math.abs(
    state.positionM.x - config.pitchWidthM / 2,
  );
  const widthFactor = clamp(
    centerDistance / (config.pitchWidthM * 0.42),
    0,
    1,
  );
  const score = (level, abilityField) =>
    tacticalLevelValue(level) * 2 + ability(state.player, abilityField) * 0.018;
  const candidates = [
    {
      action: "shoot",
      score:
        score(role.shooting, "attacking") +
        clamp((progress - 0.55) / 0.45, 0, 1) * 6 -
        widthFactor * 1.4,
    },
    {
      action: "cross",
      score:
        score(role.crossing, "passing") +
        progress * 2 +
        widthFactor * 3.2,
    },
    {
      action: "pass",
      score:
        score(role.passing, "passing") +
        ability(state.player, "decisionMaking") * 0.012,
    },
    {
      action: "carry",
      score:
        2.8 +
        score(role.carrying, "ballControl") +
        (1 - progress) * 1.8,
    },
  ];
...
function automaticBallCarrierIntent(state, team, config) {
  const role = tacticalRole(state);
  const action = automaticBallAction(state, team, config);
  const forwardDirection = team === "home" ? -1 : 1;
  const baseDistanceM = 3 + ability(state.player, "attacking") * 0.03;
  const distanceMultiplier =
    action === "pass" ? 0.42 : action === "shoot" ? 1.18 : 1;
  let targetM = add(
    state.positionM,
    point(0, forwardDirection * baseDistanceM * distanceMultiplier),
  );
  if (action === "cross" || role.preferredZone !== "balanced") {
    targetM = point(
      targetM.x + (preferredLaneX(state, config) - targetM.x) * 0.42,
      targetM.y,
    );
  }
  if (action === "shoot") {
    targetM = point(
      targetM.x + (config.pitchWidthM / 2 - targetM.x) * 0.28,
      targetM.y,
    );
  }
  return {
    behavior: action === "carry" ? "ball-carrier" : `ball-carrier-${action}`,
    intensity:
      (0.58 + ability(state.player, "decisionMaking") * 0.0022) *
      tacticalLevelMultiplier(role.carrying),
    targetM: constrainRoleTarget(state, targetM, config),
  };
}

function roleRunIntent(state, team, config) {
  const role = tacticalRole(state);
  if (role.forwardRun === "balanced") {
    return null;
  }
  if (role.forwardRun === "hold") {
    return {
      behavior: "hold-position",
      intensity: 0.52,
      targetM: constrainRoleTarget(state, state.anchorM, config, {
        verticalRange: "low",
      }),
    };
  }
  ...
  const forwardDirection = team === "home" ? -1 : 1;
  const verticalRangeM = tacticalRangeM(role.verticalRange, "vertical");
  const desiredLaneX =
    role.forwardRun === "overlap"
      ? config.pitchWidthM / 2 +
        (state.anchorM.x < config.pitchWidthM / 2 ? -1 : 1) *
          (config.pitchWidthM * 0.43)
      : config.pitchWidthM / 2 +
        (state.anchorM.x < config.pitchWidthM / 2 ? -1 : 1) *
          (config.pitchWidthM * 0.2);
  return {
    behavior: role.forwardRun,
    intensity:
      0.68 + ability(state.player, "decisionMaking") * 0.0018,
    targetM: constrainRoleTarget(
      state,
      point(
        desiredLaneX,
        state.anchorM.y + forwardDirection * verticalRangeM,
      ),
      config,
    ),
  };
...
function isBackLinePlayer(player) {
  return ["LB", "LCB", "CB", "RCB", "RB", "LWB", "RWB"].includes(
    player.role,
  );
}

function backLineStates(states, team) {
  return states.filter(
    (state) => state.player.team === team && isBackLinePlayer(state.player),
  );
}

function restDefensePriority(state) {
  if (["LCB", "CB", "RCB"].includes(state.player.role)) {
    return 0;
  }
  if (["LDM", "DM", "RDM"].includes(state.player.role)) {
    return 1;
  }
  if (tacticalRole(state).forwardRun === "hold") {
    return 2;
  }
  return 3;
}
...
function validateScenario(scenario, instructions, config) {
  ...
  for (const player of scenario.players) {
    ...
    if (player.tacticalRole !== undefined) {
      validatePlayerTacticalRole(player.tacticalRole, player.role);
    }
  }
  if (
...
function mergeConfig(overrides) {
  ...
  if (config.automaticDecisionIntervalMs % SIMULATION_TICK_MS !== 0) {
    throw new RangeError(
      "config.automaticDecisionIntervalMs must align to a simulation tick.",
    );
  }
...
function attackingAutomaticIntents(
  ...
  const restDefensePlayers = outfield
    .filter(
      (state) =>
        tacticalRole(state).forwardRun !== "overlap" &&
        tacticalRole(state).forwardRun !== "underlap",
    )
    .sort(
      (left, right) =>
        restDefensePriority(left) - restDefensePriority(right) ||
        compareById(left, right),
    )
    .slice(0, teamTactics.restDefenseCount);

  for (const state of automaticStates) {
    if (state.player.id === controlledOwnerId) {
      intents.set(
        state.player.id,
        automaticBallCarrierIntent(state, attackingTeam, config),
      );
      continue;
    }
    ...
    if (isGoalkeeper(state.player)) {
      intents.set(state.player.id, {
        ...
        targetM: constrainRoleTarget(
          state,
          goalkeeperTarget(state, ballPositionM, config),
          config,
        ),
      });
      continue;
    ...
    if (restDefensePlayers.includes(state)) {
      const behindBallY =
        attackingTeam === "home"
          ? Math.max(state.anchorM.y - 6, ballPositionM.y + 18)
          : Math.min(state.anchorM.y + 6, ballPositionM.y - 18);
      intents.set(state.player.id, {
        behavior: "rest-defense",
        intensity: 0.58,
        targetM: constrainRoleTarget(
          state,
          point(
            state.anchorM.x +
              (ballPositionM.x - state.anchorM.x) * 0.08,
            behindBallY,
          ),
          config,
        ),
    ...
    const runIntent = roleRunIntent(state, attackingTeam, config);
    if (runIntent) {
      intents.set(state.player.id, runIntent);
      continue;
    }
...
function defendingAutomaticIntents(
  ...
  const outfield = defendingStates
    ...
    .sort((left, right) => {
      const pressureDistance = (state) =>
        distance(state.positionM, ballPositionM) -
        (tacticalLevelValue(tacticalRole(state).pressing) - 1) * 4 +
        (teamTactics.offsideTrapRequested && isBackLinePlayer(state.player)
          ? 20
          : 0);
      return (
        pressureDistance(left) - pressureDistance(right) ||
        compareById(left, right)
      );
    });
  ...
  const backLine = defendingStates.filter(
    (state) => !isGoalkeeper(state.player) && isBackLinePlayer(state.player),
  );
  const backLineAnchorWidthM = Math.max(
    1,
    Math.max(...backLine.map((state) => state.anchorM.x)) -
      Math.min(...backLine.map((state) => state.anchorM.x)),
  );
  const lineYM = (teamTactics.defensiveLineY / 100) * config.pitchLengthM;

  function defensiveLineTarget(state) {
    const centeredAnchorX = state.anchorM.x - config.pitchWidthM / 2;
    const targetX =
      config.pitchWidthM / 2 +
      centeredAnchorX * (teamTactics.defensiveWidth / backLineAnchorWidthM);
    return constrainRoleTarget(
      state,
      point(targetX, lineYM),
      config,
      { lateralRange: "high", verticalRange: "high" },
    );
  }
  ...
  for (const state of defendingStates) {
    ...
    if (rank < teamTactics.pressureCount) {
      intents.set(state.player.id, {
        behavior: "pressure",
        intensity: Math.min(
          1,
          (0.78 + ability(state.player, "defending") * 0.0018) *
            tacticalLevelMultiplier(tacticalRole(state).pressing),
        ),
        targetM: constrainRoleTarget(state, ballPositionM, config),
      });
      continue;
    }
    if (rank === teamTactics.pressureCount) {
    ...
    if (isBackLinePlayer(state.player)) {
      intents.set(state.player.id, {
        behavior: teamTactics.offsideTrapRequested
          ? "offside-line"
          : "defensive-line",
        intensity: 0.66,
        targetM: defensiveLineTarget(state),
      });
      continue;
    }
    ...
    const verticalShift =
      0.08 +
      ability(state.player, "positioning") * 0.0008 +
      (tacticalLevelValue(tacticalRole(state).defensiveDepth) - 1) * 0.04;
    intents.set(state.player.id, {
      behavior: "block",
      intensity: 0.64,
      targetM: constrainRoleTarget(
...
function canActivateOffsideTrap(
  states,
  defendingTeam,
  ball,
  ballPositionM,
  ...
  const backLine = backLineStates(states, defendingTeam);
  if (
    backLine.length < 3 ||
    backLine.some((state) => state.mode !== "automatic")
  ) {
    return false;
  }
  const lineDepths = backLine.map((state) => state.positionM.y);
  const lineSpreadM = Math.max(...lineDepths) - Math.min(...lineDepths);
  if (lineSpreadM > config.pitchLengthM * 0.085) {
    return false;
  }

  return states.some(
    (state) =>
      state.player.team === defendingTeam &&
      state.mode === "automatic" &&
      !isGoalkeeper(state.player) &&
      !isBackLinePlayer(state.player) &&
      state.behavior === "pressure" &&
      distance(state.positionM, ballPositionM) <= config.pitchWidthM * 0.25,
  );
}

function applyAutomaticTargetSpacing(states, intents, config) {
  const statesById = new Map(states.map((state) => [state.player.id, state]));
  const spaced = new Map();

  function minimumSeparation(targetM, accepted) {
    if (accepted.length === 0) {
      return Infinity;
    }
    return Math.min(
      ...accepted.map((intent) => distance(targetM, intent.targetM)),
    );
  }

  function chooseSpacedTarget(state, requestedTargetM, accepted) {
    const origin = constrainRoleTarget(state, requestedTargetM, config);
    if (
      minimumSeparation(origin, accepted) >= config.automaticMinimumSpacingM
    ) {
      return origin;
    }

    const candidates = [origin];
    for (const radiusMultiplier of [1, 1.5, 2]) {
      const radiusM = config.automaticMinimumSpacingM * radiusMultiplier;
      for (let index = 0; index < 16; index += 1) {
        ...
        candidates.push(
          constrainRoleTarget(
            ...
            add(
              requestedTargetM,
              point(Math.cos(angle) * radiusM, Math.sin(angle) * radiusM),
            ),
            config,
          ),
        );
    ...
    return candidates.sort((left, right) => {
      const leftSeparation = minimumSeparation(left, accepted);
      const rightSeparation = minimumSeparation(right, accepted);
      const leftSatisfies =
        leftSeparation >= config.automaticMinimumSpacingM;
      const rightSatisfies =
        rightSeparation >= config.automaticMinimumSpacingM;
      if (leftSatisfies !== rightSatisfies) {
        return leftSatisfies ? -1 : 1;
      }
      if (leftSatisfies && rightSatisfies) {
        return (
          distance(left, requestedTargetM) -
          distance(right, requestedTargetM)
        );
      }
      if (Math.abs(leftSeparation - rightSeparation) > EPSILON) {
        return rightSeparation - leftSeparation;
      }
      return distance(left, requestedTargetM) - distance(right, requestedTargetM);
    })[0];
  }

  for (const team of ["home", "away"]) {
    const teamIntents = [...intents.entries()]
      .filter(([playerId]) => statesById.get(playerId)?.player.team === team)
      .sort(([leftId], [rightId]) => leftId.localeCompare(rightId));

    const accepted = [];
    for (const [playerId, intent] of teamIntents) {
      const state = statesById.get(playerId);
      const spacedIntent = {
        ...intent,
        targetM: chooseSpacedTarget(state, intent.targetM, accepted),
      };
      spaced.set(playerId, spacedIntent);
      accepted.push(spacedIntent);
    }
  }
...
function automaticIntents(
  ...
  possessionTeam,
  phaseElapsedMs,
  possessionTransitionElapsedMs,
  config,
) {
  if (possessionTeam === null) {
    return applyAutomaticTargetSpacing(
      states,
      looseBallAutomaticIntents(states, ballPositionM, config),
      config,
    );
  }

  const normalizedBallPosition = metersToNormalized(ballPositionM, config);
  const attackingTactics = deriveTeamTacticalState({
    team: possessionTeam,
    possessionTeam,
    ballPosition: normalizedBallPosition,
  });
  const defendingTeam = otherTeam(possessionTeam);
  const defendingTactics = deriveTeamTacticalState({
    team: defendingTeam,
    possessionTeam,
    ballPosition: normalizedBallPosition,
    possessionTransitionElapsedMs,
    offsideTrapEligible: canActivateOffsideTrap(
      states,
      defendingTeam,
      ball,
      ballPositionM,
  ...
  return applyAutomaticTargetSpacing(states, proposed, config);
}

function automaticDecisionPhaseKey(ball, possessionTeam) {
  if (ball.kind === "controlled") {
    return `controlled:${possessionTeam}:${ball.ownerId}`;
  }
  if (ball.kind === "inFlight") {
    return `inFlight:${possessionTeam}:${ball.passId}`;
  }
  return `loose:${possessionTeam ?? "none"}`;
}

function createAutomaticDecisionState() {
  return {
    intents: new Map(),
    behaviorSinceByPlayerId: new Map(),
    lastDecisionAtMs: null,
    phaseKey: null,
  };
}

function stabilizeAutomaticIntents(
  previousIntents,
  proposedIntents,
  behaviorSinceByPlayerId,
  elapsedMs,
  config,
) {
  const stabilized = new Map();
  for (const [playerId, proposed] of proposedIntents) {
    const previous = previousIntents.get(playerId);
    const behaviorSince = behaviorSinceByPlayerId.get(playerId) ?? elapsedMs;
    const tacticalBehaviors = new Set([
      "pressure",
      "cover",
      "block",
      "defensive-line",
    ...
    if (
      previous &&
      previous.behavior !== proposed.behavior &&
      tacticalBehaviors.has(previous.behavior) &&
      tacticalBehaviors.has(proposed.behavior) &&
      elapsedMs - behaviorSince < config.automaticRoleMinimumHoldMs
    ) {
      stabilized.set(playerId, previous);
      continue;
    }
    if (
      previous &&
      previous.behavior === proposed.behavior &&
      distance(previous.targetM, proposed.targetM) <
        config.automaticTargetDeadbandM
    ) {
      stabilized.set(playerId, previous);
    } else {
      stabilized.set(playerId, proposed);
      if (!previous || previous.behavior !== proposed.behavior) {
        behaviorSinceByPlayerId.set(playerId, elapsedMs);
      }
    }
  }
  return stabilized;
...
function resolveAutomaticIntents(
  decisionState,
  states,
  ball,
  ballPositionM,
  ...
  const phaseKey = automaticDecisionPhaseKey(ball, possessionTeam);
  const decisionDue =
    decisionState.lastDecisionAtMs === null ||
    decisionState.phaseKey !== phaseKey ||
    elapsedMs - decisionState.lastDecisionAtMs >=
      config.automaticDecisionIntervalMs;
  const automaticPlayerIds = new Set(
    states
      .filter((state) => state.mode === "automatic")
      .map((state) => state.player.id),
  );

  for (const playerId of decisionState.intents.keys()) {
    if (!automaticPlayerIds.has(playerId)) {
      decisionState.intents.delete(playerId);
      decisionState.behaviorSinceByPlayerId.delete(playerId);
    }
  }

  if (decisionDue) {
    const proposed = automaticIntents(
      states,
      ball,
      ballPositionM,
      possessionTeam,
    ...
    decisionState.intents = stabilizeAutomaticIntents(
      decisionState.intents,
      proposed,
      decisionState.behaviorSinceByPlayerId,
      elapsedMs,
      config,
    );
    decisionState.lastDecisionAtMs = elapsedMs;
    decisionState.phaseKey = phaseKey;
    return decisionState.intents;
  }

  const missingPlayerIds = [...automaticPlayerIds].filter(
    (playerId) => !decisionState.intents.has(playerId),
  );
  if (missingPlayerIds.length > 0) {
    ...
    for (const playerId of missingPlayerIds) {
      decisionState.intents.set(playerId, proposed.get(playerId));
    }
  }
...
function integrateAutomaticPlayer(state, intent, deltaSeconds, config) {
  ...
  const desiredSpeedMps = Math.min(
    ...
    Math.sqrt(
      2 *
        accelerationMps2(state) *
        Math.max(
          0,
          targetDistanceM - config.automaticTargetToleranceM,
        ),
    ),
  );
...
function offsideTrapIsActive(states, team) {
  const backLine = backLineStates(states, team);
  return (
    backLine.length >= 3 &&
    backLine.every(
      (state) =>
        state.mode === "automatic" && state.behavior === "offside-line",
    )
  );
}

function actualBackLineY(states, team, config) {
  const depths = backLineStates(states, team)
    .map((state) => state.positionM.y)
  ...
  const medianDepth =
    depths.length % 2 === 0
      ...
      : depths[middle];
  return metersToNormalized(point(0, medianDepth), config).y;
}
...
function createFrame(
  ...
  events,
  config,
  possessionTransitionElapsedMs = null,
) {
  ...
  const ballPosition = metersToNormalized(
    currentBallPositionM(ball, statesById),
    config,
  );
  const possessionTeam = possessionTeamForBall(ball, statesById);
  const defendingTeam =
    possessionTeam === null ? null : otherTeam(possessionTeam);
  const offsideTrapEligible =
    defendingTeam !== null &&
    canActivateOffsideTrap(
      states,
      defendingTeam,
      ball,
      currentBallPositionM(ball, statesById),
  ...
  const frameTactics = (team) => {
    const requested = deriveTeamTacticalState({
      team,
      possessionTeam,
      ballPosition,
      possessionTransitionElapsedMs,
      offsideTrapEligible:
        defendingTeam === team && offsideTrapEligible,
    });
    const offsideTrapActive = offsideTrapIsActive(states, team);
    const actualDefensiveLineY = actualBackLineY(states, team, config);
    if (offsideTrapActive || !requested.offsideTrapRequested) {
      return { ...requested, actualDefensiveLineY, offsideTrapActive };
    }
    const settledShape = deriveTeamTacticalState({
      team,
      possessionTeam,
      ballPosition,
      possessionTransitionElapsedMs,
    ...
    return {
      ...requested,
      actualDefensiveLineY,
      defensiveLineY: settledShape.defensiveLineY,
      offsideTrapActive: false,
    };
  };
  const tactics = {
    home: frameTactics("home"),
    away: frameTactics("away"),
  };
      ...
      createPlayerState(
        {
          ...
          tacticalRole: player.tacticalRole
            ? validatePlayerTacticalRole(player.tacticalRole, player.role)
            : createDefaultPlayerTacticalRole(player.role),
        },
...
function compileFlatSimulation(scenario, instructions, config) {
  ...
  let lastPossessionTeam = possessionTeam;
  let possessionPhaseStartedAtMs = 0;
  let possessionTransitionStartedAtMs = null;
  const transitionElapsedAt = (elapsedMs) =>
    possessionTransitionStartedAtMs === null
      ? null
      : elapsedMs - possessionTransitionStartedAtMs;
  const automaticDecisionState = createAutomaticDecisionState();
  const frames = [
  ...
  for (
    ...
    const intents = resolveAutomaticIntents(
      automaticDecisionState,
      states,
      ball,
      ballPositionM,
      possessionTeam,
      elapsedMs - possessionPhaseStartedAtMs,
      transitionElapsedAt(elapsedMs),
      elapsedMs,
      config,
    );
    ...
    if (nextPossessionTeam !== null) {
      if (lastPossessionTeam === null) {
        lastPossessionTeam = nextPossessionTeam;
        possessionTransitionStartedAtMs = null;
      } else if (nextPossessionTeam !== lastPossessionTeam) {
        lastPossessionTeam = nextPossessionTeam;
        possessionTransitionStartedAtMs = nextElapsedMs;
      }
    }
    frames.push(
      createFrame(
        ...
        config,
        transitionElapsedAt(nextElapsedMs),
      ),
    );
  }
...
function compileSequenceSimulation(scenario, sequences, config) {
  ...
  if (activeSequenceIndex < sequences.length) {
    ...
    timeline.endSnapshot = createFrame(
      ...
      [],
      config,
      transitionElapsedAt(scenario.durationMs),
    );
...
export function sampleSimulation(run, elapsedMs) {
  ...
  return deepClone({
    ...
    tactics: deepClone(amount < 0.5 ? lower.tactics : upper.tactics),
    events: [],
  });
}
