...
export function deriveTeamTacticalState({
  ...
  if (
    possessionTransitionElapsedMs !== null &&
    (!Number.isFinite(possessionTransitionElapsedMs) ||
      possessionTransitionElapsedMs < 0)
  ) {
    throw new RangeError(
      "possessionTransitionElapsedMs must be null or a non-negative number.",
    );
  }
  if (typeof offsideTrapEligible !== "boolean") {
    throw new TypeError("offsideTrapEligible must be a boolean.");
  }
  ...
  const transition =
    possessionTransitionElapsedMs !== null &&
    possessionTransitionElapsedMs < DEFENSIVE_TRANSITION_MS;
  const offsideTrapRequested =
    !transition &&
    offsideTrapEligible &&
