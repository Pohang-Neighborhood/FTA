...
export function nextSequentialInstructionTimeMs(
  ...
  const completedAtMs = instructions.map((instruction) =>
    completedAtMsById?.get?.(instruction.id),
  );
  if (completedAtMs.some((value) => !Number.isFinite(value))) {
    return null;
  }
  const latestTerminalOffsetMs = Math.max(
    ...completedAtMs.map((value) => value - sequenceStartedAtMs),
  );
  const candidate = latestTerminalOffsetMs + SIMULATION_TICK_MS;
  if (sequenceStartedAtMs + candidate >= durationMs) {
    return null;
  }
  return normalizeInstructionTimeMs(candidate, durationMs);
}
