...
export const TACTICAL_SEQUENCE_VERSION = 2;
...
export function validateTacticalSequences(sequences) {
  ...
  sorted.forEach((sequence, index) => {
    ...
    if (index === 0) {
      if (sequence.startAtMs !== 0) {
        throw new RangeError(`${label}.startAtMs must be 0 for the first sequence.`);
      }
    } else if (sequence.startAtMs <= sorted[index - 1].startAtMs) {
      throw new RangeError(
        `${label}.startAtMs must be later than the previous sequence.`,
      );
    }

  });

  sorted.slice(0, -1).forEach((sequence, index) => {
    const label = `sequences[${index}]`;
    const nextSequence = sorted[index + 1];
    const sequenceDurationMs = nextSequence.startAtMs - sequence.startAtMs;
    for (const instruction of sequence.instructions) {
      if (instruction.atMs >= sequenceDurationMs) {
        throw new RangeError(
          `${label}.instructions must execute before the next sequence starts.`,
        );
      }
    }
  });
...
export function removeTacticalSequence(sequences, sequenceId) {
  ...
  if (next.length > 0 && next[0].startAtMs !== 0) {
    next[0] = { ...next[0], startAtMs: 0 };
  }
...
export function reorderTacticalSequence(sequences, sequenceId, direction) {
  ...
  const currentStartAtMs = current[index].startAtMs;
  const targetStartAtMs = current[targetIndex].startAtMs;
  const reordered = current.map((sequence, sequenceIndex) => {
    if (sequenceIndex === index) {
      return { ...sequence, startAtMs: targetStartAtMs };
    }
    if (sequenceIndex === targetIndex) {
      return { ...sequence, startAtMs: currentStartAtMs };
    }
    return sequence;
  });
  return validateTacticalSequences(normalizeSequenceOrders(reordered));
}
